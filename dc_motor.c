...
static dc_motor_err_t dc_motor_device_set_frequency(dc_motor_t const *motor,
                                                    uint32_t frequency) {
  return motor->interface.device_set_frequency
             ? motor->interface.device_set_frequency(
                   motor->interface.device_user, frequency)
             : DC_MOTOR_ERR_NULL;
}

static dc_motor_err_t
dc_motor_device_set_direction(dc_motor_t const *motor,
                              dc_motor_direction_t direction) {
  return motor->interface.device_set_direction
             ? motor->interface.device_set_direction(
                   motor->interface.device_user, direction)
             : DC_MOTOR_ERR_NULL;
}

static dc_motor_err_t dc_motor_set_direction(dc_motor_t *motor,
                                             dc_motor_direction_t direction) {
...
static dc_motor_err_t dc_motor_set_frequency(dc_motor_t *motor,
                                             uint32_t frequency) {
  ...
  return dc_motor_device_set_frequency(motor, frequency);
}

static inline float32_t dc_motor_clamp_position(dc_motor_t const *motor,
                                                float32_t position) {
...
static inline float32_t dc_motor_clamp_speed(dc_motor_t const *motor,
                                             float32_t speed) {
  if (speed != 0.0F) {
...
static inline float32_t dc_motor_clamp_acceleration(dc_motor_t const *motor,
...
static inline dc_motor_direction_t
dc_motor_speed_to_direction(dc_motor_t const *motor, float32_t speed) {
  if (fabsf(speed) < motor->config.min_speed /*||
        fabsf(speed) < (motor->config.dc_change / delta_time)*/) {
    return DC_MOTOR_DIRECTION_STOP;
  }

  return speed > 0.0F ? DC_MOTOR_DIRECTION_FORWARD
                      : DC_MOTOR_DIRECTION_BACKWARD;
}

static inline uint32_t dc_motor_speed_to_frequency(dc_motor_t const *motor,
                                                   float32_t speed) {
...
static inline int64_t dc_motor_position_to_dc_count(dc_motor_t const *motor,
                                                    float32_t position) {
  assert(motor->config.dc_change > 0.0F);

  float32_t dc_count =
      dc_motor_wrap_position(position) / motor->config.dc_change;

  return (int64_t)roundf(dc_count);
}

static inline float32_t dc_motor_dc_count_to_position(dc_motor_t const *motor,
                                                      int64_t dc_count) {
  assert(motor->config.dc_change > 0.0F);

  float32_t position = (float32_t)dc_count * motor->config.dc_change;

  return dc_motor_wrap_position(position);
}
...
dc_motor_err_t dc_motor_initialize(dc_motor_t *motor,
  ...
  motor->state.dc_count = dc_motor_position_to_dc_count(motor, start_position);
...
dc_motor_err_t dc_motor_set_speed(dc_motor_t *motor, float32_t speed) {
  ...
  uint32_t frequency = dc_motor_speed_to_frequency(motor, speed);

  return dc_motor_set_frequency(motor, frequency);
}
...
float32_t dc_motor_get_position(dc_motor_t *motor) {
  ...
  float32_t position =
      dc_motor_dc_count_to_position(motor, motor->state.dc_count);
